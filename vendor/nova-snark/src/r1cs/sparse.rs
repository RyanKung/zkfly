...
/// Serializes a field vector into fixed-width little-endian representations.
pub(crate) fn pack_field_slice<F: PrimeField>(values: &[F]) -> Option<(Vec<u8>, usize)> {
  let width = F::Repr::default().as_ref().len();
  if width == 0 {
    return None;
  }
  let byte_count = values.len().checked_mul(width)?;
  let mut packed = Vec::with_capacity(byte_count);
  for value in values {
    let repr = value.to_repr();
    if repr.as_ref().len() != width {
      return None;
    }
    packed.extend_from_slice(repr.as_ref());
  }
  Some((packed, width))
}

/// Deserializes a fixed-width little-endian field vector.
pub(crate) fn unpack_field_slice<F: PrimeField>(
  packed: &[u8],
  count: usize,
  width: usize,
) -> Option<Vec<F>> {
  if width == 0 || F::Repr::default().as_ref().len() != width {
    return None;
  }
  ...
  for chunk in packed.chunks_exact(width) {
    let mut repr = F::Repr::default();
    repr.as_mut().get_mut(..width)?.copy_from_slice(chunk);
    values.push(Option::from(F::from_repr(repr))?);
  }
  Some(values)
}

/// Returns the canonical modulus representation used by a device backend.
pub(crate) fn field_modulus<F: PrimeField>() -> Vec<u8> {
  let text = F::MODULUS.strip_prefix("0x").unwrap_or(F::MODULUS);
  let mut bytes = BigUint::from_str_radix(text, 16)
    .map(|value| value.to_bytes_le())
    .unwrap_or_default();
  let width = F::Repr::default().as_ref().len();
