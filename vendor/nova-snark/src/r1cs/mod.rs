...
mod sparse;
use crate::provider::gpu::gpu_backend;
use sparse::PrecomputedSparseMatrix;
pub use sparse::SparseMatrix;
use sparse::{field_modulus, pack_field_slice, unpack_field_slice};
...
impl<E: Engine> R1CSShape<E> {
  ...
  pub fn multiply_vec(
    ...
    if let Some(output) = gpu_multiply_vec(self, z) {
      return Ok(output);
    }
  ...
  pub fn commit_T(
    ...
    let T = gpu_cross_term(&AZ, &BZ, &CZ, &W1.E, &u).unwrap_or_else(|| {
      AZ.par_iter()
        .zip(BZ.par_iter())
        .zip(CZ.par_iter())
        .zip(W1.E.par_iter())
  ...
  pub fn commit_T_relaxed(
    ...
    let error_sum = W1
      ...
      .map(|(e1, e2)| *e1 + *e2)
      .collect::<Vec<E::Scalar>>();
    let T = gpu_cross_term(&AZ, &BZ, &CZ, &error_sum, &u).unwrap_or_else(|| {
      AZ.par_iter()
        .zip(BZ.par_iter())
        .zip(CZ.par_iter())
        .zip(W1.E.par_iter())
...
/// Attempts to evaluate all three R1CS matrices through one device boundary.
fn gpu_multiply_vec<E: Engine>(
  shape: &R1CSShape<E>,
  z: &[E::Scalar],
) -> Option<(Vec<E::Scalar>, Vec<E::Scalar>, Vec<E::Scalar>)> {
  let backend = gpu_backend()?;
  let (packed_z, field_width) = pack_field_slice(z)?;
  let (packed_a, a_width) = pack_field_slice(&shape.A.data)?;
  let (packed_b, b_width) = pack_field_slice(&shape.B.data)?;
  let (packed_c, c_width) = pack_field_slice(&shape.C.data)?;
  if a_width != field_width || b_width != field_width || c_width != field_width {
    return None;
  }
  let [packed_az, packed_bz, packed_cz] = backend.spmv_three(
    [&shape.A.indptr, &shape.B.indptr, &shape.C.indptr],
    [&shape.A.indices, &shape.B.indices, &shape.C.indices],
    [&packed_a, &packed_b, &packed_c],
    &packed_z,
    field_width,
    &field_modulus::<E::Scalar>(),
  )?;
  Some((
    unpack_field_slice(&packed_az, shape.num_cons, field_width)?,
    unpack_field_slice(&packed_bz, shape.num_cons, field_width)?,
    unpack_field_slice(&packed_cz, shape.num_cons, field_width)?,
  ))
}

/// Attempts to evaluate a vector linear combination on the installed device backend.
fn gpu_linear_combination<F: PrimeField>(left: &[F], right: &[F], scalar: &F) -> Option<Vec<F>> {
  if left.len() != right.len() {
    return None;
  }
  let backend = gpu_backend()?;
  let (packed_left, width) = pack_field_slice(left)?;
  let (packed_right, right_width) = pack_field_slice(right)?;
  let (packed_scalar, scalar_width) = pack_field_slice(std::slice::from_ref(scalar))?;
  if width != right_width || width != scalar_width {
    return None;
  }
  let packed = backend.vector_linear_combination(
    &packed_left,
    &packed_right,
    &packed_scalar,
    width,
  ...
  unpack_field_slice(&packed, left.len(), width)
}

/// Attempts to evaluate a Nova cross-term on the installed device backend.
fn gpu_cross_term<F: PrimeField>(
  az: &[F],
  bz: &[F],
  cz: &[F],
  error: &[F],
  ...
  let backend = gpu_backend()?;
  let (packed_az, width) = pack_field_slice(az)?;
  let (packed_bz, bz_width) = pack_field_slice(bz)?;
  let (packed_cz, cz_width) = pack_field_slice(cz)?;
  let (packed_error, error_width) = pack_field_slice(error)?;
  let (packed_u, u_width) = pack_field_slice(std::slice::from_ref(u))?;
  if width != bz_width || width != cz_width || width != error_width || width != u_width {
    return None;
  }
  let packed = backend.cross_term(
    &packed_az,
    &packed_bz,
    &packed_cz,
    &packed_error,
  ...
  unpack_field_slice(&packed, az.len(), width)
}
...
impl<E: Engine> RelaxedR1CSWitness<E> {
  ...
  pub fn fold(
    ...
    let W = gpu_linear_combination(W1, W2, r).unwrap_or_else(|| {
      W1.par_iter()
        .zip(W2)
        .map(|(a, b)| *a + *r * *b)
        .collect::<Vec<E::Scalar>>()
    });
    let E = gpu_linear_combination(E1, T, r).unwrap_or_else(|| {
      E1.par_iter()
        .zip(T)
        .map(|(a, b)| *a + *r * *b)
        .collect::<Vec<E::Scalar>>()
  ...
  pub fn fold_relaxed(
    ...
    let first_error = gpu_linear_combination(E1, T, r).unwrap_or_else(|| {
      E1.par_iter()
        .zip(T)
        .map(|(a, b)| *a + *r * *b)
        .collect::<Vec<E::Scalar>>()
    ...
    let r_squared = *r * *r;
    let E = gpu_linear_combination(&first_error, E2, &r_squared).unwrap_or_else(|| {
      E1.par_iter()
        .zip(T)
        .zip(E2.par_iter())
        .map(|((a, b), c)| *a + *r * *b + r_squared * *c)
        .collect::<Vec<E::Scalar>>()
    });
