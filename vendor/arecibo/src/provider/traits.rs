...
/// Expands to a CUDA MSM attempt for the curves that have a GPU conversion.
#[doc(hidden)]
#[macro_export]
macro_rules! maybe_gpu_msm {
  ($curve:ty, $scalars:expr, $bases:expr, gpu) => {
    #[cfg(all(feature = "cuda", target_os = "linux", target_arch = "x86_64"))]
    if let Some(gpu_result) = crate::provider::msm::gpu_best_msm::<$curve>($scalars, $bases) {
      return gpu_result;
    }
  };
  ($curve:ty, $scalars:expr, $bases:expr) => {};
}
  ...
  (
    ...
    impl DlogGroup for $name::Point {
      ...
      fn vartime_multiscalar_mul(
        ...
        $crate::maybe_gpu_msm!($name_curve_affine, scalars, bases $(, $gpu)?);
